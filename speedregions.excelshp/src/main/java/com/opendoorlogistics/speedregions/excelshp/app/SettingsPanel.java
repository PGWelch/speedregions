...
public class SettingsPanel extends JPanel {
	...
	private static final Preferences USER_PREFERENCES = Preferences
			.userNodeForPackage(SettingsPanel.class);
	...
	public SettingsPanel() {
		...
		add(new JLabel(
				"<html>To build a road network graph for ODL Studio:"
				+ "<ol><li>Download OpenStreetMap map data in osm.pbf format from <a href=\"http://download.geofabrik.de/\">http://download.geofabrik.de/</a></li>"
				+ "<li>Choose the smallest file that covers your geographic area - e.g. state, country etc.</li>"
				+ "<li>Set (a) the location of the osm.pbf file below and (b) the output directory for the graph.</li>"
				//+ "<li>If using ODL Studio, build for only one vehicle type at a time (e.g. just car or just bike) as it only supports one vehicle type.</li>"
				+ "<li>Press OK and wait for the graph to build (this will take some minutes).</li></ol>"
				+ "<p>If you run out of memory, relaunch this graph builder tool with a higher memory configuration.</p>"
				+ "</html>"));
		addSep();

		pbf = new FileBrowserPanel(0, "Input osm.pbf file ", settings.getPbfFile(), changeCB, false, "OK",
				new FileNameExtensionFilter("OSM data (.osm.pbf)", "pbf"));
		add(pbf);

		addSep();
		outdir = new FileBrowserPanel(0, "Output directory ", settings.getOutdirectory(), changeCB,
				true, "OK");
		add(outdir);
		...
		addSep();
		excelShp = new JCheckBox("Use Excel + shapefile containing SpeedRegions",
				settings.isUseExcelShape());
		excelShp.addActionListener(actionListener);
		add(excelShp);

		// add all excel shape stuff in their own panel
		JPanel xshPanel = new JPanel();
		xshPanel.setBorder(
				BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.GRAY, 1),
						BorderFactory.createEmptyBorder(5, 5, 5, 5)));
		xshPanel.setLayout(new BoxLayout(xshPanel, BoxLayout.Y_AXIS));
		xshPanel.setAlignmentX(Component.LEFT_ALIGNMENT);
		add(xshPanel);
		
		//addSep(xshPanel);
		excel = new FileBrowserPanel(0, "Excel file ", settings.getExcelfile(), changeCB, false,
				"OK", new FileNameExtensionFilter("Excel file (xls,xlsx)", "xls", "xlsx"));
		xshPanel.add(excel);

		addSep(xshPanel);
		shp = new FileBrowserPanel(0, "Shapefile ", settings.getShapefile(), changeCB, false, "OK",
				new FileNameExtensionFilter("Shapefile (shp)", "shp"));
		xshPanel.add(shp);
		...
		configureIdCombo();
		xshPanel.add(fieldnamePanel);

		addSep(xshPanel);
		gridSizeLabel = new JLabel("Lookup grid cell length in metres (recommended 100m) ");
		gridSize = FileBrowserPanel.createTextField(Double.toString(settings.getGridCellMetres()),
				changeCB);
		...
		xshPanel.add(gridSizePanel);

		addSep(xshPanel);
		mergeRegionsBeforeBuild = new JCheckBox(
				"Merge regions before building graph (quicker, but no reports by brick)",
				settings.isMergeRegionsBeforeBuild());
		mergeRegionsBeforeBuild.addActionListener(actionListener);
		xshPanel.add(mergeRegionsBeforeBuild);
		setAllComponentsLeftAlignment(xshPanel);

		// addSep();
		// excel = new FileBrowserPanel(0, "Excel file ", settings.getExcelfile(), changeCB, false, "OK", new
		// FileNameExtensionFilter("Excel file (xls,xlsx)", "xls", "xlsx")) ;
		// add(excel);
		//
		// addSep();
		// shp = new FileBrowserPanel(0, "Shapefile ", settings.getShapefile(), changeCB, false, "OK", new
		// FileNameExtensionFilter("Shapefile (shp)", "shp")) ;
		// add(shp);
		//
		// addSep();
		// JPanel fieldnamePanel = new JPanel();
		...
		//
		// addSep();
		// mergeRegionsBeforeBuild = new JCheckBox("Merge regions before building graph (quicker, but no reports by
		// brick)", settings.isMergeRegionsBeforeBuild());
		// mergeRegionsBeforeBuild.addActionListener(actionListener);
		// add(mergeRegionsBeforeBuild);

		setAllComponentsLeftAlignment(this);
		writeSettingsToPanel();
	}

	private static void setAllComponentsLeftAlignment(JPanel panel) {
	...
	private void addVehicleTypesPanel() {
		...
		for (VehicleType type : VehicleType.values()) {
			vehicleTypes[type.ordinal()] = new JCheckBox(type.getGraphhopperName()
					+ (!type.isSpeedRegionsSupported() ? " (speed regions unsupported)" : "") + " ",
					settings.isEnabled(type));
		...
		add(new JLabel("Warning - if you're using ODL Studio you should only select one vehicle type, multiple types are unsupported."));
	}
	...
	private void configureIdCombo() {
		...
		idField.addPopupMenuListener(new PopupMenuListener() {
			...
			public void popupMenuWillBecomeVisible(PopupMenuEvent e) {
				// refresh values
				DefaultComboBoxModel<String> model = (DefaultComboBoxModel<String>) idField
						.getModel();
				model.removeAllElements();
				try {
					for (String s : ShapefileIO
							.readHeaderNames(new File(settings.getShapefile()))) {
						model.addElement(s);
					}
				} catch (Exception e2) {
	...
	private void update() {
		for (Component component : new Component[] { excel, shp, idFieldLabel, idField,
				gridSizeLabel, gridSize, mergeRegionsBeforeBuild }) {
	...
	public static AppSettings modal() {
		...
		return JOptionPane.showConfirmDialog(null, panel, "Create road network graph",
				JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION ? panel.settings : null;
	}
