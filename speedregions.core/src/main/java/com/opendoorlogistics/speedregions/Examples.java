...
public class Examples {
	...
	public static void main(String[] args) throws Exception {
		//System.out.println(createMaltaExample(0.7));
		System.out.println(SpeedRegionLookupBuilder.compileFile(createMaltaExample(0.7), 3990));
		
		
	//	UncompiledSpeedRulesFile gb = TextUtils.fromJSON(new File("examples\\GB.json"), UncompiledSpeedRulesFile.class);
	//	//CompiledSpeedRulesFile rlb = SpeedRegionLookupBuilder.compileFile(gb, 250);
	//	System.out.println(GeomUtils.toODLTable(rlb.getTree(), true));

	}

	public static void runMaltaExample() {
		// build single quadtree
		UncompiledSpeedRulesFile rules = createMaltaExample(0.75);

		CompiledSpeedRulesFile rlb = SpeedRegionLookupBuilder.compileFile(rules, 200);

		System.out.println();
		System.out.println(GeomUtils.toODLTable(rlb.getTree(), true));

		// build some random points
		SpeedRegionLookup lookup = SpeedRegionLookupBuilder.fromCompiled(rlb);
		Envelope envelope = GeomUtils.toJTS(CENTRAL_MALTA_POLYGON).getEnvelopeInternal();
		...
		for (int i = 0; i < npoints; i++) {
			...
			Point point = GeomUtils.newGeomFactory().createPoint(coordinate);
	...
	public static UncompiledSpeedRulesFile createMaltaExample(double speedMultiplier) {
